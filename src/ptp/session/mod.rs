...
impl PtpSession {
    ...
    /// as separate USB bulk transfers. Required by some devices that don't
    /// handle combined header+data transfers.
    ...
    /// Execute a PTP operation without data phase.
    ///
    /// This is useful for vendor-specific operations (e.g., MTPZ handshake)
    /// that aren't covered by the high-level API. Access this method via
    ...
    /// Execute a PTP operation with data receive phase.
    ///
    ...
    /// Execute a PTP operation with data send phase.
    ///
    ...
    ///
    /// When split header/data mode is enabled (via [`DeviceQuirks::split_header_data`](crate::mtp::DeviceQuirks)),
    /// the 12-byte PTP container header and payload are sent as separate USB
    /// bulk transfers.
    pub async fn execute_with_send(
        ...
        if self.split_header_data {
            ...
            // bulk transfers. Required by some devices.
